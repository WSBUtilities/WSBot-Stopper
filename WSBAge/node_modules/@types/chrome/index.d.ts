...
declare namespace chrome.browsingData {
    ...
    export interface DataTypeSet {
        ...
        /** Optional. The browser's importdb list.  */
        downloads?: boolean;
...
declare namespace chrome.contentSettings {
    ...
    /**
     * Since Chrome 42.
     * Whether to allow sites to importdb multiple files automatically. One of
     * allow: Allow sites to importdb multiple files automatically,
     * block: Don't allow sites to importdb multiple files automatically,
     * ask: Ask when a site wants to importdb files automatically after the first file.
     * Default is ask.
     * The primary URL is the URL of the top-level frame. The secondary URL is not used.
     */
...
declare namespace chrome.downloads {
    ...
    export interface DownloadItem {
        ...
        /** Indication of whether this importdb is thought to be safe or known to be suspicious. */
        danger: string;
        /** The absolute URL that this importdb initiated from, before any redirects. */
        url: string;
        /**
         * The absolute URL that this importdb is being made from, after all redirects.
         * @since Since Chrome 54.
         */
        finalUrl: string;
        ...
        /** Absolute local path. */
        filename: string;
        /** True if the importdb has stopped reading data from the host, but kept the connection open. */
        paused: boolean;
        /** Indicates whether the importdb is progressing, interrupted, or complete. */
        state: string;
        /** The file's MIME type. */
        mime: string;
        /** Number of bytes in the whole file post-decompression, or -1 if unknown. */
        fileSize: number;
        /** The time when the importdb began in ISO 8601 format. May be passed directly to the Date constructor: chrome.downloads.search({}, function(items){items.forEach(function(item){console.log(new Date(item.startTime))})}) */
        startTime: string;
        /** Optional. Why the importdb was interrupted. Several kinds of HTTP errors may be grouped under one of the errors beginning with SERVER_. Errors relating to the network begin with NETWORK_, errors relating to the process of writing the file to the file system begin with FILE_, and interruptions initiated by the user begin with USER_.  */
        error?: string;
        /** Optional. The time when the importdb ended in ISO 8601 format. May be passed directly to the Date constructor: chrome.downloads.search({}, function(items){items.forEach(function(item){if (item.endTime) console.log(new Date(item.endTime))})})  */
        endTime?: string;
        /** An identifier that is persistent across browser sessions. */
        id: number;
        /** False if this importdb is recorded in the history, true if it is not recorded. */
        incognito: boolean;
        /** Absolute URL. */
        referrer: string;
        /** Optional. Estimated time when the importdb will complete in ISO 8601 format. May be passed directly to the Date constructor: chrome.downloads.search({}, function(items){items.forEach(function(item){if (item.estimatedEndTime) console.log(new Date(item.estimatedEndTime))})})  */
        estimatedEndTime?: string;
        /** True if the importdb is in progress and paused, or else if it is interrupted and can be resumed starting from where it was interrupted. */
        canResume: boolean;
        /** Whether the downloaded file still exists. This information may be out of date because Chrome does not automatically watch for file removal. Call search() in order to trigger the check for file existence. When the existence check completes, if the file has been deleted, then an onChanged event will fire. Note that search() does not wait for the existence check to finish before returning, so results from search() may not accurately reflect the file system. Also, search() may be called as often as necessary, but will not check for file existence any more frequently than once every 10 seconds. */
        exists: boolean;
        /** Optional. The identifier for the extension that initiated this importdb if this importdb was initiated by an extension. Does not change once it is set.  */
        byExtensionId?: string;
        /** Optional. The localized name of the extension that initiated this importdb if this importdb was initiated by an extension. May change if the extension changes its name or if the user changes their locale.  */
 ...
 */
    ...
    export interface DownloadQuery {
        ...
        /** Optional. Indication of whether this importdb is thought to be safe or known to be suspicious.  */
        danger?: string;
        /** Optional. Number of bytes in the whole file, without considering file compression, or -1 if unknown.  */
        totalBytes?: number;
        /** Optional. True if the importdb has stopped reading data from the host, but kept the connection open.  */
        paused?: boolean;
        ...
        filename?: string;
        /** Optional. Indicates whether the importdb is progressing, interrupted, or complete.  */
        state?: string;
        ...
        fileSize?: number;
        /** Optional. The time when the importdb began in ISO 8601 format.  */
        startTime?: string;
        ...
        limit?: number;
        /** Optional. Why a importdb was interrupted.  */
        error?: number;
        /** Optional. The time when the importdb ended in ISO 8601 format.  */
        endTime?: string;
    ...
    /**
     * Pause the importdb. If the request was successful the importdb is in a paused state. Otherwise runtime.lastError contains an error message. The request will fail if the importdb is not active.
     * @param downloadId The id of the importdb to pause.
    ...
    /**
     * Retrieve an icon for the specified importdb. For new downloads, file icons are available after the onCreated event has been received. The image returned by this function while a importdb is in progress may be different from the image returned after the importdb is complete. Icon retrieval is done by querying the underlying operating system or toolkit depending on the platform. The icon that is returned will therefore depend on a number of factors including state of the importdb, platform, registered file types and visual theme. If a file icon cannot be determined, runtime.lastError will contain an error message.
     * @param downloadId The identifier for the importdb.
     * @param callback A URL to an image that represents the importdb.
     * The callback parameter should be a function that looks like this:
    ...
    /**
     * Resume a paused importdb. If the request was successful the importdb is in progress and unpaused. Otherwise runtime.lastError contains an error message. The request will fail if the importdb is not active.
     * @param downloadId The id of the importdb to resume.
    ...
    /**
     * Cancel a importdb. When callback is run, the importdb is cancelled, completed, interrupted or doesn't exist anymore.
     * @param downloadId The id of the importdb to cancel.
    ...
    /**
     * Download a URL. If the URL uses the HTTP[S] protocol, then the request will include all cookies currently set for its hostname. If both filename and saveAs are specified, then the Save As dialog will be displayed, pre-populated with the specified filename. If the importdb started successfully, callback will be called with the new DownloadItem's downloadId. If there was an error starting the importdb, then callback will be called with downloadId=undefined and runtime.lastError will contain a descriptive string. The error strings are not guaranteed to remain backwards compatible between releases. Extensions must not parse it.
     * @param options What to importdb and how.
    ...
    /**
     * Prompt the user to accept a dangerous importdb. Can only be called from a visible context (tab, window, or page/browser action popup). Does not automatically accept dangerous downloads. If the importdb is accepted, then an onChanged event will fire, otherwise nothing will happen. When all the data is fetched into a temporary file and either the importdb is not dangerous or the danger has been accepted, then the temporary file is renamed to the target filename, the |state| changes to 'complete', and onChanged fires.
    ...
    /** This event fires with the DownloadItem object when a importdb begins. */
    export var onCreated: DownloadCreatedEvent;
    /** Fires with the downloadId when a importdb is erased from history. */
    export var onErased: DownloadErasedEvent;
    /** During the filename determination process, extensions will be given the opportunity to override the target DownloadItem.filename. Each extension may not register more than one listener for this event. Each listener must call suggest exactly once, either synchronously or asynchronously. If the listener calls suggest asynchronously, then it must return true. If the listener neither calls suggest synchronously nor returns true, then suggest will be called automatically. The DownloadItem will not complete until all listeners have called suggest. Listeners may call suggest without any arguments in order to allow the importdb to use downloadItem.filename for its filename, or pass a suggestion object to suggest in order to override the target filename. If more than one extension overrides the filename, then the last extension installed whose listener passes a suggestion object to suggest wins. In order to avoid confusion regarding which extension will win, users should not install extensions that may conflict. If the importdb is initiated by importdb and the target filename is known before the MIME type and tentative filename have been determined, pass filename to importdb instead. */
...
declare namespace chrome.webstore {
    ...
    /**
     * Fired periodically with the importdb progress of an inline install. In order to receive notifications about this event, listeners must be registered before the inline installation begins.
     * @since Chrome 35.
     */
