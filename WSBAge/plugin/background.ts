    ...
    if (port.name === "cscript") {
        port.onMessage.addListener(message => {
            if (message.request === "terms") {
                ...
                let dbRequest: IDBOpenDBRequest = indexedDB.open("WSBInfo", 1);
                ...
                dbRequest.onsuccess = ev => {
                    ...
                    let tx: IDBTransaction = db.transaction(['users'], 'readonly');
                    ...
                    for (let i = 0; i < message.users.length; i++) {
                        let request: IDBRequest = store.get(message.users[i]);
                        request.onsuccess = () => {
